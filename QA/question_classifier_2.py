...
# fine-tuned bert model as classifier
...
def cal_classifier_output():
    ...
    for item in test_question_dict:
        ...
        result_type = pred_model.predict(question)
        test_question_dict[item]["result_type"] = int(result_type)

    """保存dict为json格式到本地"""
    with open("json/classifier_output_2.json", 'w', encoding='utf-8') as f:
        json.dump(test_question_dict, f, ensure_ascii=False)


def test_classifier():
    # 载入问题类型
    question_type_list = [int(i) for i in open("E:/project/TrainKGQA/QA/config/used_question_type.txt", "r",
                                               encoding="utf-8").readline().split(" ")]

    # 载入JSON数据
    json_data = json.load(open("json/data.json", "r", encoding="utf-8"))

    # 测试数据，返回字典train_question={1: [question1, question2, ...], 2:[], }
    test_question_dict = {}

    # 对每个question_type键创建空列表
    for question_type in question_type_list:
        test_question_dict[question_type] = []

    for item in json_data["test"]:
        if item['question_type'] in question_type_list:
            test_question_dict[item['question_type']].append(item['question'])
    ...
    for question_type in question_type_list:
        for question in test_question_dict[question_type]:
            # 进行问题分类
            result_type = pred_model.predict(question)
            group = {"question": question, "question_type": question_type, "result_type": int(result_type)}
            test_result_list.append(group)

    # 遍历每种问题，分别计算准确率Precision，召回率Recall
    # 实际为X类有t个
    # 预测为X类有m个
    # 实际为X类并且预测为X类有n个
    # 准确率就是n/m，召回率就是n/t
    ...
    for question_type in question_type_list:
        ...
        precision_recall_list.append([question_type, precision, recall, f1])

    precision_recall_table = pd.DataFrame(precision_recall_list, columns=['type', 'precision', 'recall', 'F1'])
    precision_recall_table.to_csv("classifier_f1_2.csv")
...
if __name__ == "__main__":
    ...
    # type = '11'
    # pred_model = SentencePredictionModel(MODEL_PATH, bert_model)
    # predictions, true_labels = pred_model.predict(text, type)

    # print(predictions)
    # print(true_labels)

    # test_classifier()
    cal_classifier_output()
